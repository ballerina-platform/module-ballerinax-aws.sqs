...
public function main(string... args) returns error? {
    ...
    sqs:CreateQueueResponse|error response1 = sqsClient->createQueue("myQueue");
    ...
    sqs:ReceiveMessageResponse response3 = check sqsClient->receiveMessage(queueResourcePath, 1, 600, 2, attributeNames, messageAttributeNames);
    ...
    sqs:DeleteMessageResponse response4 = check sqsClient->deleteMessage(queueResourcePath, receivedReceiptHandler);
    if (response4 is sqs:DeleteMessageResponse) {
        log:printInfo("Successfully deleted the message from the queue.");
    }

    // Delete the queue
    sqs:DeleteQueueResponse response5 = check sqsClient->deleteQueue(queueResourcePath);
